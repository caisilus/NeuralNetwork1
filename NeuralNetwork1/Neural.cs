...
namespace NeuralNetwork1
{
    ...
    public class Sample
    {
        ...
        /// </summary>
        public double[] Error { get; set; } = null;
        ...
        public FigureType ProcessPrediction(double[] neuralOutput)
        {
            ...
            int maxEvaluationIndex = 0;
            for (int i = 0; i < Output.Length; ++i)
            {
                Error[i] = Output[i] - ExpectedOutput(i);
                if (Output[i] > Output[maxEvaluationIndex]) 
                    maxEvaluationIndex = i;
            }

            //  Распознанный класс - это класс с максимальной оценкой
            recognizedClass = (FigureType) maxEvaluationIndex;
            return recognizedClass;
        }

        private double ExpectedOutput(int classIndex)
        {
            return classIndex == (int) actualClass ? 1 : 0;
        }
